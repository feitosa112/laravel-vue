    ...
    <nav class="collapse position-absolute navbar navbar-vertical navbar-light align-items-start p-0 bg-light" id="navbar-vertical" style="width: calc(100% - 30px); z-index: 999;">
        <div class="navbar-nav w-100">
            <div class="nav-item dropdown dropright"  v-for="category in allCategories">
                <router-link :to="{name:'getProductsWithCategory',params:{category_id:category.id}}"  v-if="category.with_sub_category.length === 0" class="nav-link dropdown-toggle" data-toggle="dropdown">{{ category.name }}</router-link>
                ...
                <div class="dropdown-menu position-absolute rounded-0 border-0 m-0"  v-for="subCategory in category.with_sub_category">
                    ...
                    <router-link :to="{name:'getProductsWithSubCategory',params:{subcategory_id:subCategory.id}}" class="dropdown-item">{{ subCategory.name }}</router-link>
